...
import DataUtilityModal from './components/supervisor/DataUtilityModal';
import MergeFacilityModal from './components/supervisor/MergeFacilityModal';
import StockRateDetailModal from './components/supervisor/StockRateDetailModal';
import BillNumberChangeModal from './components/supervisor/BillNumberChangeModal';
import TransferDataModal from './components/supervisor/TransferDataModal';
import TransferOtherDataModal from './components/supervisor/TransferOtherDataModal';
import ChallanProblemModal from './components/supervisor/ChallanProblemModal';
import ChangeBillsModal from './components/supervisor/ChangeBillsModal';
import SalesBillDeleteModal from './components/supervisor/SalesBillDeleteModal';
import PurchaseBillDeleteModal from './components/supervisor/PurchaseBillDeleteModal';
          ...
          <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
            ...
            <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "76%", height: "50%", border: "2px solid #22d3ee", borderRadius: "8px", boxShadow: "0 0 0 9999px rgba(0,0,0,0.0)" }}>
              ...
              {[["top", "left"], ["top", "right"], ["bottom", "left"], ["bottom", "right"]].map(([v, h], i) => (
                <div key={i} style={{
                  position: "absolute", [v]: "-2px", [h]: "-2px", width: "22px", height: "22px",
...
export default function OwnerPanel() {
  const {
    ...
    openPurchaseReturnForm, updatePurchaseReturnItem, handleSavePurchaseReturn, handleDeletePurchaseReturn,
  ...
  // ── Sales Bill Enhanced States (Legacy Visual InfoSoft Parity) ──
  ...
  const [salesPreviewBill, setSalesPreviewBill] = useState<any>(null);
  ...
  // ── Purchase Return Enhanced States (Legacy Visual InfoSoft Parity) ──
  ...
  const [purchaseReturnPreview, setPurchaseReturnPreview] = useState<any>(null);
  ...
  const [purchaseReturnItemSearch, setPurchaseReturnItemSearch] = useState<any>({});
  const [purchaseReturnItemHighlight, setPurchaseReturnItemHighlight] = useState<any>({});
  const [purchaseReturnItemDropdown, setPurchaseReturnItemDropdown] = useState<number | null>(null);
  const [purchaseReturnDropdownPos, setPurchaseReturnDropdownPos] = useState({ top: 0, left: 0, width: 240 });

  // ── Purchase Bill Enhanced States (Legacy Visual InfoSoft Parity) ──
  ...
  const [purchasePreviewBill, setPurchasePreviewBill] = useState<any>(null);
  ...
  // ── Masters View Modes ──
  const [kitViewMode, setKitViewMode] = useState('editor');
  const [contractEmpViewMode, setContractEmpViewMode] = useState('editor');
  ...
  // (PurchaseBillDelete state moved to PurchaseBillDeleteModal component)
  // -- WIP Feature State --
  ...
  // ── Sales Bill Enhanced Helpers (Visual InfoSoft Legacy Parity) ──
  const [dupSearch, setDupSearch] = useState("");
  const [listDrawerSearch, setListDrawerSearch] = useState("");

  const holdCurrentSalesBill = () => {
    const validItems = salesItems.filter((si: any) => si.itemId || si.itemName);
    if (!validItems.length && !salesForm.patientName) {
      showToast("Nothing to hold in current bill", "info");
      return;
    }
    const totalAmt = salesItems.reduce((s: number, si: any) => s + num(si.amount || 0), 0);
    const held = {
      id: uid(),
      heldAt: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      date: salesForm.date || today(),
      patientName: salesForm.patientName || "Walk-in Customer",
      salesForm: { ...salesForm },
      salesItems: [...salesItems],
      isReturn: !!isReturn,
      totalAmt
    };
    setPendingSalesBills(prev => [held, ...prev]);
    setSalesForm(emptySalesForm());
    setSalesItems([emptySalesItem()]);
    showToast(`Bill held in Pending Queue! (${pendingSalesBills.length + 1} pending)`);
  };

  const resumePendingSalesBill = (pendingBill: any) => {
    setSalesForm({ ...emptySalesForm(), ...pendingBill.salesForm });
    setSalesItems(pendingBill.salesItems && pendingBill.salesItems.length > 0 ? pendingBill.salesItems : [emptySalesItem()]);
    setIsReturn(!!pendingBill.isReturn);
    setPendingSalesBills(prev => prev.filter(b => b.id !== pendingBill.id));
    setShowPendingModal(false);
    showToast(`Resumed bill for ${pendingBill.patientName || 'Customer'}`);
  };

  const discardPendingSalesBill = (id: string) => {
    setPendingSalesBills(prev => prev.filter(b => b.id !== id));
    showToast("Pending bill discarded");
  };

  const handlePrevBill = () => {
    if (!salesBills.length) { showToast("No sales bills found", "info"); return; }
    if (!salesForm.id) {
      openSalesForm(salesBills[0].isReturn, salesBills[0]);
    } else {
      const idx = salesBills.findIndex((b: any) => String(b.id) === String(salesForm.id));
      if (idx < salesBills.length - 1) {
        openSalesForm(salesBills[idx + 1].isReturn, salesBills[idx + 1]);
      } else {
        showToast("Already at oldest bill", "info");
      }
    }
  };
  ...
  const handleNextBill = () => {
    if (!salesBills.length) { showToast("No sales bills found", "info"); return; }
    if (!salesForm.id) {
      showToast("Already at new bill", "info");
      return;
    }
    const idx = salesBills.findIndex((b: any) => String(b.id) === String(salesForm.id));
    if (idx > 0) {
      ...
      openSalesForm(false);
      showToast("Opened new fresh bill");
    }
  };

  const duplicateSalesBill = (targetBill: any) => {
    if (!targetBill) return;
    setSalesForm({
      ...emptySalesForm(),
      patientName: targetBill.patientName || "",
      patientArea: targetBill.patientArea || "",
      doctorName: targetBill.doctorName || "",
      mobile: targetBill.mobile || "",
      address: targetBill.address || "",
      paymentMode: targetBill.paymentMode || "cash",
      discount: targetBill.discount || targetBill.lessDisc || "0",
      salesMan: targetBill.salesMan || "",
      quotation: false
    });
    const dupItems = (targetBill.items || []).map((si: any) => ({
      ...si,
      amount: calcSalesItemAmt(si)
    }));
    setSalesItems(dupItems.length > 0 ? dupItems : [emptySalesItem()]);
    setShowDuplicateModal(false);
    showToast(`Duplicated ${dupItems.length} items from Bill #${targetBill.billNo}`);
  };

  // ── Purchase Bill Helper Functions (Parity with Legacy Visual InfoSoft) ──
  const holdCurrentPurchaseBill = () => {
    const validItems = purchaseItems.filter((pi: any) => pi.itemId || pi.itemName);
    if (!validItems.length) { showToast("Add at least 1 item to hold purchase bill", "error"); return; }
    const totalAmt = purchaseItems.reduce((s: any, pi: any) => s + (pi.amount || 0), 0);
    const held = {
      id: uid(),
      heldAt: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      date: purchaseForm.billDate || purchaseForm.entryDate || today(),
      partyName: purchaseForm.partyName || "Supplier",
      purchaseForm: { ...purchaseForm },
      purchaseItems: [...purchaseItems],
      totalAmt
    };
    setPendingPurchaseBills((prev: any) => [held, ...prev]);
    setPurchaseForm(emptyPurchaseForm());
    setPurchaseItems([emptyPurchaseItem()]);
    showToast(`Purchase bill held in Pending Queue! (${pendingPurchaseBills.length + 1} pending)`);
  };

  const resumePendingPurchaseBill = (pendingBill: any) => {
    setPurchaseForm({ ...emptyPurchaseForm(), ...pendingBill.purchaseForm });
    setPurchaseItems(pendingBill.purchaseItems && pendingBill.purchaseItems.length > 0 ? pendingBill.purchaseItems : [emptyPurchaseItem()]);
    setPendingPurchaseBills((prev: any) => prev.filter((b: any) => b.id !== pendingBill.id));
    setShowPendingPurchaseModal(false);
    showToast(`Resumed purchase bill for ${pendingBill.partyName || 'Supplier'}`);
  };

  const discardPendingPurchaseBill = (id: string) => {
    setPendingPurchaseBills((prev: any) => prev.filter((b: any) => b.id !== id));
    showToast("Pending purchase bill discarded");
  };

  const handlePrevPurchaseBill = () => {
    if (!purchaseBills.length) { showToast("No purchase bills found", "info"); return; }
    if (!purchaseForm.id) {
      openPurchaseForm(purchaseBills[0]);
    } else {
      const idx = purchaseBills.findIndex((b: any) => String(b.id) === String(purchaseForm.id));
      if (idx < purchaseBills.length - 1) {
        openPurchaseForm(purchaseBills[idx + 1]);
      } else {
        showToast("Already at oldest purchase bill", "info");
      }
    }
  };

  const handleNextPurchaseBill = () => {
    if (!purchaseBills.length) { showToast("No purchase bills found", "info"); return; }
    if (!purchaseForm.id) {
      showToast("Already at new purchase bill", "info");
      return;
    }
    const idx = purchaseBills.findIndex((b: any) => String(b.id) === String(purchaseForm.id));
    if (idx > 0) {
      ...
      showToast("Opened new fresh purchase bill");
    }
  };

  const duplicatePurchaseBill = (targetBill: any) => {
    if (!targetBill) return;
    const nextEntry = (purchaseBills.length > 0 ? Math.max(...purchaseBills.map((b: any) => parseInt(b.entryNo) || 0)) : 0) + 1;
    setPurchaseForm({
      ...emptyPurchaseForm(),
      entryNo: String(nextEntry),
      billSeries: targetBill.billSeries || "G",
      partyName: targetBill.partyName || "",
      supplierId: targetBill.supplierId || "",
      billDate: today(),
      entryDate: today(),
      taxType: targetBill.taxType || "exclusive",
      paymentMode: targetBill.paymentMode || "cash",
      taxZone: targetBill.taxZone || "sgst_ugst",
      remarks: targetBill.remarks ? `Copy of ${targetBill.billNo || targetBill.entryNo}` : "",
      billMsg: targetBill.billMsg || ""
    });
    const dupItems = (targetBill.items || []).map((pi: any) => ({
      ...emptyPurchaseItem(),
      ...pi,
      batchNo: "",
      expiryDate: "",
    ...
    setPurchaseItems(dupItems.length > 0 ? dupItems : [emptyPurchaseItem()]);
    setShowDuplicatePurchaseModal(false);
    showToast(`Duplicated ${dupItems.length} items from Bill #${targetBill.billNo || targetBill.entryNo}`);
  };

  const printPurchaseVoucher = (bill: any) => {
    setPurchasePreviewBill(bill);
    setShowPurchasePreviewModal(true);
  };

  // ── Purchase Return Helper Functions (Legacy Visual InfoSoft Parity) ──
  const holdCurrentPurchaseReturn = () => {
    const validItems = purchaseReturnItems.filter((pi: any) => pi.itemId || pi.itemName);
    if (!validItems.length) { showToast("Add at least 1 item to hold purchase return", "error"); return; }
    const totalAmt = purchaseReturnItems.reduce((s: any, pi: any) => s + (pi.amount || 0), 0);
    const held = {
      id: uid(),
      heldAt: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      date: purchaseReturnForm.entryDate || purchaseReturnForm.date || today(),
      partyName: purchaseReturnForm.partyName || purchaseReturnForm.supplierName || "Supplier",
      purchaseReturnForm: { ...purchaseReturnForm },
      purchaseReturnItems: [...purchaseReturnItems],
      totalAmt
    };
    setPendingPurchaseReturns((prev: any) => [held, ...prev]);
    setPurchaseReturnForm(emptyPurchaseReturnForm());
    setPurchaseReturnItems([emptyPurchaseReturnItem()]);
    showToast(`Purchase return held in Pending Queue! (${pendingPurchaseReturns.length + 1} pending)`);
  };

  const resumePendingPurchaseReturn = (pendingRet: any) => {
    setPurchaseReturnForm({ ...emptyPurchaseReturnForm(), ...pendingRet.purchaseReturnForm });
    setPurchaseReturnItems(pendingRet.purchaseReturnItems && pendingRet.purchaseReturnItems.length > 0 ? pendingRet.purchaseReturnItems : [emptyPurchaseReturnItem()]);
    setPendingPurchaseReturns((prev: any) => prev.filter((r: any) => r.id !== pendingRet.id));
    setShowPendingPurchaseReturnModal(false);
    showToast(`Resumed purchase return for ${pendingRet.partyName || 'Supplier'}`);
  };

  const discardPendingPurchaseReturn = (id: string) => {
    setPendingPurchaseReturns((prev: any) => prev.filter((r: any) => r.id !== id));
    showToast("Pending purchase return discarded");
  };

  const handlePrevPurchaseReturn = () => {
    if (!purchaseReturns.length) { showToast("No purchase returns found", "info"); return; }
    if (!purchaseReturnForm.id) {
      openPurchaseReturnForm(purchaseReturns[0]);
    } else {
      const idx = purchaseReturns.findIndex((r: any) => String(r.id) === String(purchaseReturnForm.id));
      if (idx < purchaseReturns.length - 1) {
        openPurchaseReturnForm(purchaseReturns[idx + 1]);
      } else {
        showToast("Already at oldest purchase return", "info");
      }
    }
  };

  const handleNextPurchaseReturn = () => {
    if (!purchaseReturns.length) { showToast("No purchase returns found", "info"); return; }
    if (!purchaseReturnForm.id) {
      showToast("Already at new purchase return", "info");
      return;
    }
    const idx = purchaseReturns.findIndex((r: any) => String(r.id) === String(purchaseReturnForm.id));
    if (idx > 0) {
      ...
      openPurchaseReturnForm(null);
      showToast("Opened new fresh purchase return");
    }
  };

  const duplicatePurchaseReturn = (targetRet: any) => {
    if (!targetRet) return;
    const nextEntry = (purchaseReturns.length > 0 ? Math.max(...purchaseReturns.map((r: any) => parseInt(r.entryNo || r.returnNo) || 0)) : 0) + 1;
    setPurchaseReturnForm({
      ...emptyPurchaseReturnForm(),
      entryNo: String(nextEntry),
      billSeries: targetRet.billSeries || "G",
      partyName: targetRet.partyName || targetRet.supplierName || "",
      supplierId: targetRet.supplierId || "",
      entryDate: today(),
      billDate: today(),
      billType: targetRet.billType || "TAX",
      taxZone: targetRet.taxZone || "sgst_ugst",
      refBillNo: targetRet.refBillNo || "",
      reason: targetRet.reason || "Expired",
      detail: targetRet.detail || "",
      billMsg: targetRet.billMsg || ""
    });
    const dupItems = (targetRet.items || []).map((pi: any) => {
      const found = items.find((i: any) => i.id === pi.itemId || i.name === pi.itemName);
      const r = {
        ...emptyPurchaseReturnItem(),
        ...pi,
        unit: pi.unit || found?.unit || "",
        rate: pi.rate || pi.ptr || found?.pRate || "",
        gst: pi.gst || found?.gst || 5,
        location: pi.location || found?.location || found?.rack || ""
      };
      const rate = num(r.rate), qty = int(r.qty), gst = num(r.gst), disc = num(r.disc);
      const base = rate * qty, discAmt = base * disc / 100, taxable = base - discAmt;
      r.discAmt = discAmt;
      r.base = taxable;
      r.amount = taxable + taxable * gst / 100;
      return r;
    });
    setPurchaseReturnItems(dupItems.length > 0 ? dupItems : [emptyPurchaseReturnItem()]);
    setShowDuplicatePurchaseReturnModal(false);
    showToast(`Duplicated ${dupItems.length} items from Return #${targetRet.returnNo || targetRet.entryNo}`);
  };

  const printPurchaseReturnVoucher = (ret: any) => {
    setPurchaseReturnPreview(ret);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "row", flex: 1, minHeight: 0, overflow: "hidden", width: "100%", background: "var(--bg-body)" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, height: "calc(100vh - 50px)", maxHeight: "calc(100vh - 50px)", overflow: "hidden" }}>
        ...
        ══════════════════════════════════════════ */}
          {/* ══════════════════════════════════════════ */}
          {/* OWNER: MASTERS (MODULARIZED COMPONENT)     */}
          {/* ══════════════════════════════════════════ */}
          {isOwner && activeSection === "masters" && (
            <MastersModule
              ownerSubTab={ownerSubTab}
              setOwnerSubTab={setOwnerSubTab}
              setShowWipModal={setShowWipModal}
              kitViewMode={kitViewMode}
              setKitViewMode={setKitViewMode}
              contractEmpViewMode={contractEmpViewMode}
              setContractEmpViewMode={setContractEmpViewMode}
            />
          )}
          ...
          {/* DATA UTILITY MODAL */}
          {/* SUPERVISOR — DATA UTILITY (MODULAR COMPONENT) */}
          {showDataUtility && <DataUtilityModal onClose={() => setShowDataUtility(false)} />}


          {/* ═══════════════════════════════════════════════════════════════ */}
          {/* SUPERVISOR — DATA MERGE FACILITY (100% FULLSCREEN MODERN THEME) */}
          {/* ═══════════════════════════════════════════════════════════════ */}
          {/* SUPERVISOR — DATA MERGE FACILITY (MODULAR COMPONENT) */}
          {showMergeFacility && <MergeFacilityModal onClose={() => setShowMergeFacility(false)} />}


          {/* ═══════════════════════════════════════════════════════════════ */}
          {/* SUPERVISOR — STOCK RATE DETAIL & PRICE LEDGER (100% FULLSCREEN) */}
          {/* ═══════════════════════════════════════════════════════════════ */}
          {/* SUPERVISOR — STOCK RATE DETAIL (MODULAR COMPONENT) */}
          {showStockRateDetail && <StockRateDetailModal onClose={() => setShowStockRateDetail(false)} />}


          {/* BILL LOCK MODAL - FULLSCREEN */}
          {/* SUPERVISOR — BILL LOCK MANAGEMENT (MODULAR COMPONENT) */}
          {showLockBill && <LockBillModal onClose={() => setShowLockBill(false)} />}

          {/* USERWISE CHANGES FULLSCREEN MODAL (SUPERVISOR) */}
          {/* SUPERVISOR — USERWISE CHANGES & AUDIT TRAIL (MODULAR COMPONENT) */}
          {showUserwiseChanges && <UserwiseChangesModal onClose={() => setShowUserwiseChanges(false)} />}


          {/* ═══════════════════════════════════════════════════════════════ */}
          {/* SUPERVISOR — BILL NUMBER CHANGE & RENUMBERING (100% FULLSCREEN) */}
          {/* ═══════════════════════════════════════════════════════════════ */}
          {/* SUPERVISOR — BILL NUMBER CHANGE (MODULAR COMPONENT) */}
          {showBillNumberChange && <BillNumberChangeModal onClose={() => setShowBillNumberChange(false)} />}
      ...
      ══════════════════════════════════════════ */}
        ...
        ══════════════════════════════════════════ */}
          ...
          {/* ═══════════════════════════════════════════════════════════════ */}
          {/* ═══════════════════════════════════════════════════════════════ */}
          {/* SUPERVISOR MODALS (MODULARIZED COMPONENTS)                     */}
          {/* ═══════════════════════════════════════════════════════════════ */}
          <TransferDataModal isOpen={showTransferData} onClose={() => setShowTransferData(false)} />
          <TransferOtherDataModal isOpen={showTransferOtherData} onClose={() => setShowTransferOtherData(false)} />
          <ChallanProblemModal isOpen={showChallanProblem} onClose={() => setShowChallanProblem(false)} />
          <ChangeBillsModal isOpen={showChangeBills} onClose={() => setShowChangeBills(false)} />
          <SalesBillDeleteModal isOpen={showSalesBillDelete} onClose={() => setShowSalesBillDelete(false)} />
          <PurchaseBillDeleteModal isOpen={showPurchaseBillDelete} onClose={() => setShowPurchaseBillDelete(false)} />
